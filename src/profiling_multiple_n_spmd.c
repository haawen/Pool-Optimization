...
const char *file_name = "N_spmd.csv";
...
void summarize_profile(Profile *profile, const char *func_name, const char *part_name, int iteration, int n, FILE *file)
{
    fprintf(file, "\"%s\",%s,%d,%llu,%d\n", func_name, part_name, iteration, profile->cycles_cumulative, n);
}
...
int main()
{
    ...
    fprintf(csv, "Function,Iteration,N,Cycles\n");
    fclose(csv);
