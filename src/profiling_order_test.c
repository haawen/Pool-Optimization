...
void call_function(const char* name, CollideBallsFn collide_fn) {
    // Determine which CSV file to use based on the current test
    const char* csv_file;
    if (strcmp(name, "Basic Implementation") == 0) {
        csv_file = "profiling_order1.csv";
    } else if (strcmp(name, "Code Motion") == 0) {
        if (strcmp(name, "Basic Implementation") == 0) {
            csv_file = "profiling_order2.csv";
        } else {
            csv_file = "profiling_order1.csv";
        }
    } else { // SIMD
        csv_file = "profiling_order3.csv";
    }
...
int main() {
    // First order: Basic -> Code Motion -> SIMD
    FILE* csv1 = fopen("profiling_order1.csv", "w");
    if (csv1 == NULL) {
        perror("Failed to open CSV file");
        return 0;
    }
    fprintf(csv1, "Function,Section,Test Case,Iteration,Cycles\n");
    fclose(csv1);
    ...
    int result1 = UNITY_END();

    // Second order: Code Motion -> Basic -> SIMD
    FILE* csv2 = fopen("profiling_order2.csv", "w");
    if (csv2 == NULL) {
        perror("Failed to open CSV file");
        return 0;
    }
    fprintf(csv2, "Function,Section,Test Case,Iteration,Cycles\n");
    fclose(csv2);
    ...
    UNITY_BEGIN();
        ...
        RUN_TEST(test_collide_balls_basic);
        RUN_TEST(test_collide_balls_simd);
    int result2 = UNITY_END();

    // Third order: SIMD -> Basic -> Code Motion
    FILE* csv3 = fopen("profiling_order3.csv", "w");
    if (csv3 == NULL) {
        perror("Failed to open CSV file");
        return 0;
    }
    fprintf(csv3, "Function,Section,Test Case,Iteration,Cycles\n");
    fclose(csv3);
