...
void call_function(const char *name, CollideBallsFnSPMD collide_fn)
{
    ...
    FILE *csv = fopen("flops_spmd.csv", "a");
    if (csv == NULL)
    {
    ...
    fprintf(csv, "%s,%s,%ld,%ld,%ld,%ld,%ld,%ld\n", name, "Delta", profiles[3].flops, profiles[3].memory * sizeof(double), profiles[3].ADDS, profiles[3].MULS, profiles[3].DIVS, profiles[3].SQRT);
    fprintf(csv, "%s,%s,%ld,%ld,%ld,%ld,%ld,%ld\n", name, "Velocity", profiles[4].flops, profiles[4].memory * sizeof(double), profiles[4].ADDS, profiles[4].MULS, profiles[4].DIVS, profiles[4].SQRT);
    fprintf(csv, "%s,%s,%ld,%ld,%ld,%ld,%ld,%ld\n", name, "Transform to World Frame", profiles[5].flops, profiles[5].memory * sizeof(double), profiles[5].ADDS, profiles[5].MULS, profiles[5].DIVS, profiles[5].SQRT);
...
int main()
{
    FILE *csv = fopen("flops_spmd.csv", "w");
    if (csv == NULL)
    {
        perror("Failed to open CSV file");
        return 0;
    }
    fprintf(csv, "Function,Section,Flops,Memory,ADDS,MULS,DIVS,SQRT\n");
    fclose(csv);
